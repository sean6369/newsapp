    ...
    >
      <AnimatePresence mode="wait" custom={direction.current} initial={false}>
        ...
        >
          ...
          <div className="flex items-center gap-2 text-xs text-muted mb-2 pr-8 max-md:pr-14">
            ...
            {active.readingTime > 0 && (
              <>
                <span className="hidden md:inline">&middot;</span>
                <span className="hidden md:inline">{active.readingTime} min</span>
              </>
            )}
          </div>
          ...
          <div className="flex items-center mt-auto pt-3">
            <span className={`flex-1 text-xs font-medium uppercase tracking-wider ${feedColor[active.feed] || "text-muted"}`}>
              {active.feed}
            </span>
      ...
      {hasSwitcher && (
        <>
          ...
          >
            <svg width="20" height="48" viewBox="0 0 12 28" fill="none">
