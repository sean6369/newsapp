...
export default async function FeedPage({ searchParams }: { searchParams: SearchParams }) {
  const [cookieStore, params, readMarks] = await Promise.all([
    cookies(),
    searchParams,
    getReadMarksEnabled(),
  ]);
  ...
  // is not revalidated on arrival or on mount — `revalidateIfStale: false`
  // suppresses both — so the newest day would serve this snapshot until the
  // reader next came back to the tab. That is the one day the crawler is still
